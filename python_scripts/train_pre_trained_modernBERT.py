# type: ignore
"""
Resume training from a saved HF Trainer checkpoint.

Identical setup to train_modernBERT.py: same model types, training types,
datasets, collators, trainers, callbacks. The only difference is that
trainer.train(resume_from_checkpoint=ckpt_path) restores model weights,
optimizer state, LR scheduler, RNG state, and step counter from disk
instead of starting from step 0 with fresh state.

Use this when a long training run was interrupted (timeout/crash/preempt)
and you want to pick up exactly where it left off.
"""
import os
...
class CustomTrainingArguments(TrainingArguments):
    ...
    ckpt_path: str = field(
        default="/gpfs/data/brandeslab/Models/modernBERT_1B/checkpoint-1000000",
        metadata={"help": "Path to the HF Trainer checkpoint directory to resume from"},
    )
    max_steps: int = field(
...
def main():
    ...
    if not os.path.isdir(training_args.ckpt_path):
        raise FileNotFoundError(
            f"ckpt_path does not exist or is not a directory: {training_args.ckpt_path}"
        )
    ...
    # Build model
    # =========================================================================
    # Note: we still build a fresh model here. HF Trainer.train(resume_from_checkpoint=...)
    # will overwrite these weights with the saved state when training begins.
    ...
    print_rank0(f"Resuming training from checkpoint: {training_args.ckpt_path}")
    trainer.train(resume_from_checkpoint=training_args.ckpt_path)
